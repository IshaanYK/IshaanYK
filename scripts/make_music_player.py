...
svg = f"""<svg xmlns="http://www.w3.org/2000/svg" width="{W}" height="{H}" viewBox="0 0 {W} {H}" font-family="ui-monospace, SFMono-Regular, Menlo, Consolas, monospace">
  ...
  <rect width="{W}" height="{H}" rx="8" fill="url(#mpbg)"/>
  ...
  <!-- Progress Bar: looping animation to show it's "playing" -->
  <g transform="translate(480, 31)">
    <!-- Progress border -->
    <rect x="0" y="0" width="200" height="12" rx="3" fill="#161b22" stroke="{FRAME}" stroke-width="1"/>
    
    <!-- Progress fill (animated width) -->
    <rect x="1" y="1" width="0" height="10" rx="2" fill="{GREEN}">
      <animate attributeName="width" from="0" to="198" dur="15s" repeatCount="indefinite" />
    </rect>
    
    <!-- Timer -->
    <text x="210" y="11" fill="{MUTED}" font-size="11">LIVE</text>
  </g>
  
  <!-- Floating Play Button / Playing Status -->
  <g transform="translate(740, 37)">
    <circle cx="0" cy="0" r="12" fill="{GREEN}" opacity="0.8">
      <animate attributeName="opacity" values="0.8;0.4;0.8" dur="2s" repeatCount="indefinite"/>
    </circle>
    <!-- Play arrow -->
    <polygon points="-3,-5 6,0 -3,5" fill="#0d1117"/>
    <text x="20" y="4" fill="{INK}" font-size="12" font-weight="bold">Click to Play</text>
  </g>
</svg>
"""
